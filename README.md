...
3. Extract and install:
   ...
   ```
4. **Gatekeeper** — because the binary is not codesigned with an Apple
   Developer ID, macOS will refuse to run it the first time with
   *"findmouse can't be opened because Apple cannot check it for malicious
   software."* Strip the quarantine attribute:
   ```sh
   xattr -dr com.apple.quarantine /usr/local/bin/findmouse
   ```
